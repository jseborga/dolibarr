...
/**
 *	\file       htdocs/core/modules/pettycash/doc/doc_generic_pettycash_odt.modules.php
 *	\ingroup    pettycash
 *	\brief      File of class to build ODT documents for pettycashs
 */

dol_include_once('/pettycash/core/modules/pettycash/modules_pettycash.php');
 ...
 */
class doc_generic_pettycash_odt extends ModelePDFMyObject
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function info($langs)
	{
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=pc_pettycash/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->pettycash->dir_output) {
			...
			$dir = $conf->pettycash->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->pettycash->dir_temp);
				if (!is_writable($conf->pettycash->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->pettycash->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->pettycash->dir_temp,
