...
/**
 * mailing_mailinglist_pettycash
 */
class mailing_mailing_pettycash_selector1 extends MailingTargets
{
	// CHANGE THIS: Put here a name not already used
	public $name = 'mailing_pettycash_selector1';
	...
	public $enabled = 'isModEnabled("pettycash")';
	...
	/**
	 * @var string 	String with the name of icon for pettycash. Can be an image filename like 'object_pettycash.png' of a font awesome code 'fa-...'.
	 */
	...
	{
		return '<a href="'.dol_buildpath('/pettycash/pettycash_card.php', 1).'?id='.$id.'">'.img_object('', "generic").'</a>';
	}
	...
	public function add_to_target($mailing_id)
	{
		...
		$sql .= " FROM ".$this->db->prefix()."pettycash";
			...
			while ($i < $num) {
				...
				if ($old != $obj->email) {
					$target[$j] = array(
						...
						'source_type' => 'pettycash@pettycash'
					);
					$old = $obj->email;
					$j++;
				}
	...
	public function getNbOfRecipients($sql = '')
	{
		...
		$sql .= " FROM ".$this->db->prefix()."pettycash as p";
		$sql .= " WHERE email IS NOT NULL AND email <> ''";
