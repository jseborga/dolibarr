...
/**
 *    \file       htdocs/modulebuilder/template/pettycash_card.php
 *    \ingroup    pettycash
 *    \brief      Page to create/edit/view pettycash
 */
...
dol_include_once('/pettycash/lib/pc_pettycash.lib.php');
...
$langs->loadLangs(array("pettycash@pettycash", "other"));
...
$diroutputmassaction = $conf->pettycash->dir_output.'/temp/massgeneration/'.$user->id;
...
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('pettycash', 'pettycash', 'read');
	$permissiontoadd = $user->hasRight('pettycash', 'pettycash', 'write'); // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
	$permissiontodelete = $user->hasRight('pettycash', 'pettycash', 'delete') || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
	$permissionnote = $user->hasRight('pettycash', 'pettycash', 'write'); // Used by the include of actions_setnotes.inc.php
	$permissiondellink = $user->hasRight('pettycash', 'pettycash', 'write'); // Used by the include of actions_dellink.inc.php
} else {
	$permissiontoread = 1;
...
$upload_dir = $conf->pettycash->multidir_output[isset($object->entity) ? $object->entity : 1].'/pettycash';
...
if (empty($reshook)) {
	$backurlforlist = dol_buildpath('/pettycash/pettycash_list.php', 1);
	...
	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) {
				...
				$backtopage = dol_buildpath('/pettycash/pettycash_card.php', 1).'?id='.((!empty($id) && $id > 0) ? $id : '__ID__');
			}
		}
	}
...
llxHeader('', $title, $help_url, '', 0, 0, '', '', '', 'mod-pettycash page-card');
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create'))) {
	$head = pettycashPrepareHead($object);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/pettycash/pettycash_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	*/
	...
	if ($action != 'presend') {
		...
		if ($includedocgeneration) {
			...
			$filedir = $conf->pettycash->dir_output.'/'.$object->element.'/'.$objref;
			$urlsource = $_SERVER["PHP_SELF"]."?id=".$object->id;
			$genallowed = $permissiontoread; // If you can read, you can build the PDF to read content
			$delallowed = $permissiontoadd; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('pettycash:MyObject', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$tmparray = $form->showLinkToObjectBlock($object, array(), array('pettycash'), 1);
		...
		$morehtmlcenter = dolGetButtonTitle($langs->trans('SeeAll'), '', 'fa fa-bars imgforviewmode', dol_buildpath('/pettycash/pettycash_agenda.php', 1).'?id='.$object->id);
	...
	$diroutput = $conf->pettycash->dir_output;
	$trackid = 'pettycash'.$object->id;
